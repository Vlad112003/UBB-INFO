...
import ro.ubbcluj.cs.map.lab72nd.domain.Entity;
import ro.ubbcluj.cs.map.lab72nd.domain.FriendShip;
import ro.ubbcluj.cs.map.lab72nd.domain.StringFriendship;
...
//        }
        ...
        while(iterator.hasNext()) {
            ...
            String friendsFrom = (fr.getFriendsFrom().format(formatter));
...
//
    ...
    public void onPressUpdate(){
        try {
            User userInit = userTableView.getSelectionModel().getSelectedItem();
            String errors = "";
            String userFirstName = firstNameTextField.getText();
            if(userFirstName.isEmpty())
                errors += "Invalid first name!\n";
            ...
            if(!userEmail.isEmpty())
                errors += "Email change not allowed!\n";
            String userPassword = passwordTextField.getText();
            if(!errors.isEmpty())
                MessageAlert.showErrorMessage(null, errors);
            else {
                User user = new User(userFirstName, userLastName, userInit.getEmail(), userPassword);
    ...
    public void onPressMostSociableCommunity(){
        if(service.mostSociableCommunity() == null)
            MessageAlert.showMessage(null, "No communities!");
        else MessageAlert.showMessage(null, "Most sociable community: " + service.mostSociableCommunity());
    }
    ...
    public void onPressShowFriendshipsByUserinMonth(){
        try {
            ...
            if(monthInt < 1 || monthInt > 12)
                ...
                if(StreamSupport.stream(friends.spliterator(), false).count() == 0)
