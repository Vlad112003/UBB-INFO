package ro.ubbcluj.cs.map.lab82nd.service;
...
import static ro.ubbcluj.cs.map.lab82nd.domain.FriendRequest.*;
...
//    private final Repository<UUID, User> userRepo;
    ...
    private final UserPageDatabase userRepo;
     ...
     * @param messageRepo         - the repository for the messages
     * @param messageValidator    - the validator for the messages
     */
    public Service(UserPageDatabase userRepo, Validator<User> userValidator, Repository<UUID, FriendShip> friendshipRepo, Validator<FriendShip> friendshipValidator, Repository<UUID, Message> messageRepo, Validator<Message> messageValidator) {
    ...
    public boolean createFriendship(String email1, String email2) {
        ...
        var frenship = new FriendShip(u1, u2, PENDING);
        try {
...
//        return null;
    ...
    public Iterable<FriendShip> getFriendsforUser(User user){
        Iterable<FriendShip> l = friendshipRepo.findAll();
        List<FriendShip> list = new ArrayList<>();
        l.forEach(f -> {
            if(f.getUser1().getEmail().equals(user.getEmail()) || f.getUser2().getEmail().equals(user.getEmail())){
                list.add(f);
            }
        });
        return list;
    ...
    public User getUserById(User user2) {
        Iterable<User> it = userRepo.findAll();
        AtomicReference<User> found = new AtomicReference<>();
        it.forEach(u -> {
    ...
    public void setStatus(FriendShip friendship, FriendRequest fr){
        FriendShip frenship = new FriendShip(friendship.getId(), friendship.getUser1(), friendship.getUser2(), friendship.getFriendsFrom(), fr);
        friendshipRepo.update(frenship);
    }
    ...
    public FriendShip getFriendshipByUsers(User user1, User user2){
        ...
        AtomicReference<FriendShip> found = new AtomicReference<>();
        l.forEach(f -> {
            if(f.getUser1().getId().equals(user1.getId()) && f.getUser2().getId().equals(user2.getId()))
                found.set(f);
        });
        return found.get();
    }

    public User getUserByStringEmail(String email){
        Iterable<User> it = userRepo.findAll();
        AtomicReference<User> found = new AtomicReference<>();
        it.forEach(u -> {
            if (u.getEmail().equals(email))
    ...
    public FriendShip getFriendshipByFriendName(User user, String firstName2, String lastName2){
        Iterable<FriendShip> list = this.getFriendsforUser(user);
        for(FriendShip f : list){
            if(f.getUser1().getFirstName().equals(firstName2) && f.getUser1().getLastName().equals(lastName2))
                return f;
            if(f.getUser2().getFirstName().equals(firstName2) && f.getUser2().getLastName().equals(lastName2))
                return f;
        }
        return null;
    }

    public ArrayList<StringFriend> convertToFriendNameForUser(Iterable<FriendShip> friends, User user){
        ArrayList<StringFriend> friendos = new ArrayList<>();
        for (FriendShip friendship : friends) {
            if (friendship.getUser1().getId().equals(user.getId())) {
                User friend = this.getUserById(friendship.getUser2());
                String firstName = friend.getFirstName();
                String lastName = friend.getLastName();
                String status = friendship.getStatus().toString();
                StringFriend sfs = new StringFriend(firstName, lastName, status);
                friendos.add(sfs);
            } else {
                User friend = this.getUserById(friendship.getUser1());
                String firstName = friend.getFirstName();
                String lastName = friend.getLastName();
                String status = friendship.getStatus().toString();
                StringFriend sfs = new StringFriend(firstName, lastName, status);
    ...
    public boolean sendMessage(User user1, User user2, String message){
        UUID user1_id = user1.getId();
        UUID user2_id = user2.getId();
        Message m = new Message(user1_id, user2_id, message, (UUID) null);
        try {
            this.messageValidator.validate(m);
            messageRepo.save(m);
        } catch (Exception e) {
            return false;
        }
    ...
    public boolean sendReply(User user1, User user2, String message, UUID replyTo){
        UUID user1_id = user1.getId();
        UUID user2_id = user2.getId();
        Message m = new Message(user1_id, user2_id, message, replyTo);
        try {
            this.messageValidator.validate(m);
            messageRepo.save(m);
        } catch (Exception e) {
    ...
    public Iterable<Message> getMessagesToUserForUser(User user1, User user2){
        Iterable<Message> l = messageRepo.findAll();
        List<Message> list = new ArrayList<>();
        l.forEach(m -> {
            if(m.getTo().equals(user2.getId()) && m.getFrom().equals(user1.getId()))
                list.add(m);
        });
        return list;
    }

    public Iterable<Message> getMessagesFromUserForUser(User user1, User user2){
        Iterable<Message> l = messageRepo.findAll();
        List<Message> list = new ArrayList<>();
        l.forEach(m -> {
            if(m.getFrom().equals(user2.getId()) && m.getTo().equals(user1.getId()))
                list.add(m);
        });
        return list;
    }

    public UUID getMessageIdByMessage(String message){
        Iterable<Message> l = messageRepo.findAll();
        UUID found = null;
        for (Message m : l){
