...
//    Repository<UUID, User> userRepo;
    ...
    public Optional<FriendShip> findOne(UUID id){
        ...
        try(Connection connection = DriverManager.getConnection(url, username, password);
            ...
            friendship = new FriendShip(u1, u2, parsed, status);
...
//                System.out.println(from);
                ...
                FriendShip friendship = new FriendShip(u1, u2, parsed, status);
                friendship.setId(id_);
    ...
    public Optional<FriendShip> update(FriendShip entity) {
        ...
        FriendShip frenship = friendShip.get();
        if(frenship.getId().equals(entity.getId()))
        {
            try (Connection connection = DriverManager.getConnection(url, username, password)) {
                Statement statement = connection.createStatement();
                String sql = "UPDATE " + tableName + " set id_user1 = '" + entity.getUser1().getId().toString() + "' " + " where id = '" + frenship.getId().toString() + "';";
                statement.executeUpdate(sql);
                sql = "UPDATE " + tableName + " set id_user2 = '" + entity.getUser2().getId().toString() + "' " + " where id = '" + frenship.getId().toString() + "';";
                statement.executeUpdate(sql);
                String unparsed = entity.getFriendsFrom().format(formatter);
                LocalDateTime friendsFrom = LocalDateTime.parse(unparsed,formatter);
                sql = "UPDATE " + tableName + " set friendsFrom = '" + friendsFrom.toString() + "' " + " where id = '" + frenship.getId().toString() + "';";
                statement.executeUpdate(sql);
                sql = "UPDATE " + tableName + " set status = '" + entity.getStatus().toString() + "' " + " where id = '" + frenship.getId().toString() + "';";
                statement.executeUpdate(sql);
            }
            catch (SQLException e) {
                System.out.println(e);
