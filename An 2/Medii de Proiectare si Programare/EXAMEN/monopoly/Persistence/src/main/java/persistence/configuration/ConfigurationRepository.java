...
public class ConfigurationRepository implements IConfigurationRepository {
    ...
    public Configuration getConfigurationById(Integer id) {
        logger.traceEntry("Retrieving conf");
        try (Session session = sessionFactory.openSession()) {
            Transaction transaction = null;
            try {
                transaction = session.beginTransaction();
                Configuration conf = session.createQuery("from Configuration where id = :id", Configuration.class)
                        .setParameter("id", id).setMaxResults(1)
                        .uniqueResult();
                transaction.commit();
                logger.traceExit("Successfully retrieved conf");
                return conf;
            } catch (RuntimeException ex) {
                if (transaction != null)
                    transaction.rollback();
                logger.error("Error while conf: " + ex);
            }
        }
        logger.traceExit("Error");
        return null;
