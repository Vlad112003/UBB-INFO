...
public class RpcWorker implements Runnable, IObserver {
    ...
    private Response handleGET_ONE_BY_USERNAME(Request request) {
        System.out.println("Login request ..." + request.type());
        System.out.println("Received request: " + request.data().toString());
        String username = (String) request.data();
        try {
            Game found = service.getOneByUsername(username);
            return new Response.Builder().type(ResponseType.OK).data(found).build();
        } catch (Exception e) {
            connected = false;
            return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
    ...
    private Response handleGET_CONF_BY_ID(Request request) {
        System.out.println("Login request ..." + request.type());
        System.out.println("Received request: " + request.data().toString());
        Integer id = (Integer) request.data();
        try {
            Configuration found = service.getConfigurationById(id);
            return new Response.Builder().type(ResponseType.OK).data(found).build();
        } catch (Exception e) {
            connected = false;
            return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
    ...
    private Response handleCHOOSE_POSITION(Request request) {
        ...
        try {
            String output = service.choosePosition(data[0], data[1]);
