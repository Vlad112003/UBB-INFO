...
//        client.gameFinished(finishedGame);
    ...
    public Game getOneByUsername(String username) throws Exception {
        initializeConnection();
        Request request = new Request.Builder().type(RequestType.GET_ONE_BY_USERNAME).data(username).build();
        sendRequest(request);
        Response response = readResponse();
        switch (response.type()) {
            case OK -> {
    ...
    @Override
    public String choosePosition(int id, int n) throws Exception {
        Request request = new Request.Builder().type(RequestType.CHOOSE_POSITION).data(new int[]{id, n}).build();
        sendRequest(request);
        Response response = readResponse();
        switch (response.type()) {
            case OK -> {
