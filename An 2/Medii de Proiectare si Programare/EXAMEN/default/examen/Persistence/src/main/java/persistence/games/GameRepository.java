...
public class GameRepository implements IGameRepository {
    ...
    @Override
    public Game startGame(String player,Integer pct,  String date, String time) {
        logger.traceEntry("Starting game");
        try (org.hibernate.Session session = sessionFactory.openSession()) {
            org.hibernate.Transaction transaction = null;
            try {
                transaction = session.beginTransaction();
                Game game = new Game(player,  date, time,pct, new ArrayList<>());
                session.save(game);
                transaction.commit();
    ...
    public Game getById(Integer id) {
        logger.traceEntry("Find a game by id");
        try(Session session=sessionFactory.openSession()){
            Transaction transaction=null;
            try {
                transaction=session.beginTransaction();
    ...
    @Override
    public Game getOneByUsername(String username) {
        logger.traceEntry("Retrieving games by username");
        try (Session session = sessionFactory.openSession()) {
            Transaction transaction = null;
            try {
                transaction = session.beginTransaction();
                Game game = session.createQuery("from Game where player = :username order by id desc", Game.class)
    ...
    @Override
    public Collection<Game> getByUsernameOrdered(String username) {
        logger.traceEntry("Retrieving games by username");
        try (Session session = sessionFactory.openSession()) {
            Transaction transaction = null;
            try {
                transaction = session.beginTransaction();
                Collection<Game> games = session.createQuery("from Game where player = :username order by durata DESC", Game.class)
                        .setParameter("username", username).list();
                transaction.commit();
                logger.traceExit("Successfully retrieved games by username");
                return games;
