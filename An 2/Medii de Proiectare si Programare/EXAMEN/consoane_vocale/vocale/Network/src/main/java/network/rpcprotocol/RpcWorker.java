...
public class RpcWorker implements Runnable, IObserver {
    ...
    private Response handleCHOOSE_LITERA(Request request) {
        System.out.println("Choose position request ..." + request.type());
        System.out.println("Received request: " + request.data().toString());
        String [] keywords=(String[]) request.data();
        try {
            String output = service.chooseLitera(Integer.parseInt(keywords[0]), keywords[1]);
    ...
    private Response handleGET_GAME(Request request) {
        ...
        try {
            Game output = service.getGame(id);
            return new Response.Builder().type(ResponseType.OK).data(output).build();
        } catch (Exception e) {
            connected = false;
            return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
