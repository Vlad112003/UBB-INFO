...
// Adăugăm elementul în ordine crescătoare
    Nod* nodCurent = prim;
    Nod* nodAnterior = nullptr;

// Căutăm poziția de inserare a elementului
    while (nodCurent != nullptr && rel(nodCurent->data, elem)) {
        nodAnterior = nodCurent;
        nodCurent = nodCurent->urm;
    }

// Inserăm noul nod înaintea nodului curent sau la sfârșitul listei
    if (nodAnterior == nullptr) {
        // Inserare la începutul listei
        newNod->urm = prim;
        prim = newNod;
    } else {
        // Inserare în interiorul sau la sfârșitul listei
        nodAnterior->urm = newNod;
        newNod->urm = nodCurent;
    }
...
// Caz 2: Elementul este primul element
    if (p == prim) {
        prim = prim->urm;
        delete p;
        size--;
...
// Caz 4: Elementul nu este nici primul, nici ultimul
    anterior->urm = p->urm;
    delete p;
    size--;
    return true;
