...
$param3 = $args[2] # Matrix rows
$param4 = $args[3] # Matrix cols
$param5 = $args[4] # Function Name
...
for ($i = 0; $i -lt $param2; $i++) {
    ...
    # Capture the output of the CUDA program and filter only the line with 'milliseconds'
    $a = (cmd /c .\$param1 $param3 $param4 $param5 $param6 2>&1 | Select-String "milliseconds")
...
if (!(Test-Path outC.csv)) {
    ...
    # Scrie date in csv
    Set-Content outC.csv 'Matrix Rows,Matrix Cols,Tip Convolutie,Timp executie mediu'
}

# Append the average time to the CSV file
Add-Content outC.csv "$($param3),$($param4),$($param5),$($media)"